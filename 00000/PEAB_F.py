...
def gerar_relatorio_consolidado(modelo: Pipeline, X_test: pd.DataFrame, y_test: pd.Series, X_train: pd.DataFrame, y_train: pd.Series, class_names: List[str], nome_dataset: str, t_plus: float, t_minus: float):
    ...
    caminho_relatorio = os.path.join("report",f"PEAB_F_{nome_dataset}.txt")
    ...
    for i in range(len(X_test)):
        ...
        if rejeitada:
            ...
            expl_final, adicoes, remocoes, premissa_vencedora = encontrar_explicacao_otimizada_para_rejeitada(modelo, inst_df, expl_inicial_f1, expl_inicial_f2, X_train, t_plus, t_minus, class_names, log_instancia_atual)
    ...
    with open(caminho_relatorio, "w", encoding="utf-8") as f:
        f.write("================================================================================\n")
        f.write("       RELATÓRIO DE ANÁLISE DE PI-EXPLICAÇÕES (MÉTODO FORMAL BIDIRECIONAL)\n")
        f.write("================================================================================\n\n")
