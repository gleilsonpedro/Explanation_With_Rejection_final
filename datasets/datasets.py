...
def carregar_dataset(nome_dataset: str) -> Tuple[Optional[pd.DataFrame], Optional[pd.Series], Optional[List[str]]]: #
    try:
        ...
        if nome_dataset == 'sonar':
            ...
            # Substitui os zeros inválidos por NaN tem alguns dados que são impossíveis como glixcose = 0
            ...
            print(f"Linhas originais: {len(data_df)}, Linhas após remoção: {len(data_df.dropna())}")
            print(data_df['target'].value_counts(normalize=True))  # Antes porcentagem de diferença das classes
            print(data_df.dropna()['target'].value_counts(normalize=True))  # Depois a mesma coisa

            # Remove TODAS as linhas que contêm NaN.
            # O 'inplace=True' garante que a modificação seja feita no próprio data_df.
            data_df.dropna(inplace=True)
            
            # Reseta o índice para ser uma sequência limpa (0, 1, 2, ...),
            # o que evita o erro de alinhamento. maior buxo que deu antes
            ...
            # DEBUG: Linha para verificar se ainda existem NaNs em X. O resultado deve ser 0.
            # Você pode remover esta linha depois que confirmar que o erro foi resolvido.
            print(f"VERIFICAÇÃO DE DADOS AUSENTES NO PIMA (DEVE SER 0): {X.isnull().sum().sum()}")
            ...
            try:
                # Baixa o conteúdo do arquivo zip em memória
                r = requests.get(url)
                r.raise_for_status() # Lança um erro se o download falhar
                
                # Abre o arquivo zip a partir dos bytes baixados
                z = zipfile.ZipFile(io.BytesIO(r.content))
                
                # Lê o arquivo específico 'column_2C.dat' de dentro do zip
            ...
            # O resto do seu código de processamento permanece o mesmo
...
def selecionar_dataset_e_classe() -> Tuple[Optional[str], Optional[str], Optional[pd.DataFrame], Optional[pd.Series], Optional[List[str]]]: #
    ...
    | [0] Sonar (208x60x2)               | [1] Pima Diabetes (392x8x2)     |
    | [2] Breast Cancer (569x30x2)       | [3] Creditcard Fraud (Amostra)  |
    | [4] Banknote Auth (1372x4x2)       | [5] Heart Disease (297x13x2)    |
    | [6] Wine Quality (Red) (1599x11x2) | [7] Wine (178x13x3)             |
    | [8] Vertebral Column (310x6x2)     | [9] Seeds (210x7x3)             |
    | [Q] SAIR                                                             |
    |----------------------------------------------------------------------|
    '''
    print(menu)
    nomes_datasets = [
        'sonar', 'pima_indians_diabetes', 'breast_cancer', # 'iris' virou 'sonar'
        'creditcard', 'banknote', 'heart_disease',
        'wine_quality', 'wine', 'vertebral_column', 'seeds'
    ]
    while True:
