...
# --- Módulos do projeto ---
try:
    import auxiliary_files.svm_explainer
    import auxiliary_files.utility
...
if __name__ == '__main__':
    ...
    if dados_completos is not None:
        ...
        final_targets = auxiliary_files.utility.check_targets(np.array(alvos_prontos))
        ...
        t_upper, t_lower = auxiliary_files.utility.find_thresholds(clf, X_train, y_train, wr=wr_param)
        metricas['t_upper'], metricas['t_lower'] = t_upper, t_lower
        
        pos_idx, neg_idx, rej_idx = auxiliary_files.utility.find_indexes(clf, X_exp, t_upper, t_lower)
        all_explanations = {}
        ...
        if len(neg_idx) > 0:
            explanations = auxiliary_files.svm_explainer.svm_explanation_binary(
        ...
        if len(rej_idx) > 0:
            explanations = auxiliary_files.svm_explainer.svm_explanation_rejected(
                dual_coef=clf.dual_coef_, support_vectors=clf.support_vectors_, intercept=clf.intercept_,
                t_lower=t_lower, t_upper=t_upper, lower_bound=scaled_data.min(), upper_bound=scaled_data.max(),
        ...
        metricas['acuracia_com_rejeicao'] = auxiliary_files.utility.calculate_accuracy(clf, t_upper, t_lower, X_test, y_test) * 100
