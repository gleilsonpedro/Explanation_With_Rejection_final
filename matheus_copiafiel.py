...
# --- Definição única do random_state ---
RANDOM_STATE = 107  # Para consistência com o ipynb

def formatar_log(metricas):
    # (Mantém a função original, sem alterações)
...
if __name__ == '__main__':
    ...
    if dados_completos is not None:
        ...
        # Split antes do escalonamento para evitar vazamento
        X_train, X_test, y_train, y_test = train_test_split(
            dados_prontos, alvos_prontos, test_size=test_size_atual, random_state=RANDOM_STATE, stratify=alvos_prontos
        )
        
        # Escalonamento após o split
        scaler = MinMaxScaler()
        X_train = scaler.fit_transform(X_train)  # Ajusta e transforma apenas o treino
        X_test = scaler.transform(X_test)       # Transforma o teste com o scaler do treino
        metricas['num_features'] = X_train.shape[1]
        ...
        t_upper, t_lower = utility.find_thresholds(clf, X_train, y_train, wr=wr_param)
        ...
        if len(neg_idx) > 0:
            explanations = svm_explainer.svm_explanation_binary(
                ...
                t_lower=t_lower, t_upper=t_upper, lower_bound=X_train.min(), upper_bound=X_train.max(),  # Usa min/max do treino
        ...
        metricas['acuracia_com_rejeicao'] = utility.calculate_accuracy(clf, t_upper, t_lower, X_test, y_test) * 100
        ...
        update_method_results("mateus", dataset_key, results_data)
