...
if __name__ == '__main__':
    # Dicionário de configuração com test_size E wr para cada dataset
    params_config = {
    ...
    if dados_completos is not None:
        ...
        if 'subsample_size' in config_atual and isinstance(dados_completos, (pd.DataFrame, pd.Series)):
        ...
        print(f"\n--- Iniciando análise (MATEUS - SVM) para: {nome_relatorio} ---")
        metricas = {'dataset_name': nome_relatorio, 'test_size': test_size_atual, 'rejection_cost': wr_param[0]}
        
        dados_numpy = np.array(dados_prontos)
        final_targets = utility.check_targets(np.array(alvos_prontos))
        metricas['num_features'] = dados_numpy.shape[1]
        
        # --- ORDEM CORRIGIDA: 1. DIVIDIR ---
        X_train, X_test, y_train, y_test = train_test_split(
            dados_numpy, final_targets, test_size=test_size_atual, random_state=107, stratify=final_targets
        )
        metricas['total_instancias_teste'] = len(y_test)

        # --- ORDEM CORRIGIDA: 2. ESCALAR ---
        scaler = MinMaxScaler()
        X_train_scaled = scaler.fit_transform(X_train)
        X_test_scaled = scaler.transform(X_test)

        # --- ORDEM CORRIGIDA: 3. TREINAR ---
        clf = svm.SVC(kernel='linear', C=1.0)
        clf.fit(X_train_scaled, y_train)

        # --- 4. AVALIAR E EXPLICAR ---
        metricas['acuracia_sem_rejeicao'] = metrics.accuracy_score(y_test, clf.predict(X_test_scaled)) * 100
        metricas['intercepto'] = clf.intercept_[0]

        # A função de thresholds usa os dados de TREINO escalados para aprender
        t_upper, t_lower = utility.find_thresholds(clf, X_train_scaled, y_train, wr=wr_param)
        metricas['t_upper'], metricas['t_lower'] = t_upper, t_lower
        
        # As explicações são geradas para o conjunto de TESTE escalado
        X_exp, y_exp = X_test_scaled, y_test
        
        # Garantir que os dados estejam no intervalo [0, 1] 
        # A função np.clip() força quaisquer valores ligeiramente > 1.0 para 1.0
        # e quaisquer valores < 0.0 para 0.0, resolvendo o erro do pulp.
        X_exp = np.clip(X_exp, 0, 1)

        pos_idx, neg_idx, rej_idx = utility.find_indexes(clf, X_exp, t_upper, t_lower)
        all_explanations = {}
        
        tempo_total_explicacoes = 0
        
        # Geração de explicações para classe Negativa
        start_time_neg = time.time()
        if len(neg_idx) > 0:
            explanations = svm_explainer.svm_explanation_binary(
                dual_coef=clf.dual_coef_, support_vectors=clf.support_vectors_, intercept=clf.intercept_,
                t_lower=t_lower, t_upper=t_upper, lower_bound = 0.0, upper_bound = 1.0,
        ...
        # Geração de explicações para classe Positiva
        ...
        # Geração de explicações para classe Rejeitada
        start_time_rej = time.time()
        if len(rej_idx) > 0:
            explanations = svm_explainer.svm_explanation_rejected(
                dual_coef=clf.dual_coef_, support_vectors=clf.support_vectors_, intercept=clf.intercept_,
                t_lower=t_lower, t_upper=t_upper, lower_bound = 0.0, upper_bound = 1.0,
        ...
        # Cálculo de métricas de desempenho com rejeição
        ...
        metricas['acuracia_com_rejeicao'] = utility.calculate_accuracy(clf, t_upper, t_lower, X_test_scaled, y_test) * 100
        
        # Coleta de estatísticas das explicações
        ...
        for idx, exp in all_explanations.items():
            exp_len = len(exp)
            if idx in pos_idx: exp_lengths_pos.append(exp_len)
            elif idx in neg_idx: exp_lengths_neg.append(exp_len)
            elif idx in rej_idx: exp_lengths_rej.append(exp_len)
        ...
        for class_key, exp_lengths in [("positive", exp_lengths_pos), ("negative", exp_lengths_neg), ("rejected", exp_lengths_rej)]:
            ...
            if exp_lengths:
                stats.update({
                    'min_length': int(np.min(exp_lengths)), 'mean_length': float(np.mean(exp_lengths)),
                    'max_length': int(np.max(exp_lengths)), 'std_length': float(np.std(exp_lengths))
                })
            else:
        ...
        # Geração do log em .txt
        log_final = formatar_log(metricas)
        if not os.path.exists('report'): os.makedirs('report')
        caminho_arquivo = os.path.join('report', f'mateus_{nome_relatorio}.txt')
        with open(caminho_arquivo, 'w', encoding='utf-8') as f: f.write(log_final)
        print(f"\nANÁLISE CONCLUÍDA. Relatório salvo em: {caminho_arquivo}")
        
        # Geração e salvamento do JSON
        dataset_key = nome_dataset_original
        
# --- INÍCIO DO BLOCO CORRIGIDO ---
        
# --- INÍCIO DO BLOCO NOVO E COMPLETO ---

# 1. Estruturar os dados para o JSON, espelhando o formato antigo
results_data = {
    "config": {
        "dataset_name": nome_dataset_original,
        "classifier": "SVM",
        "kernel": "linear",
        "test_size": metricas.get('test_size', 0),
        "random_state": RANDOM_STATE, # Usando a constante definida no topo do script
        "rejection_cost_wr": metricas.get('rejection_cost', 0)
    },
    "thresholds": {
        "t_upper": metricas.get('t_upper', 0),
        "t_lower": metricas.get('t_lower', 0)
    },
    "performance": {
        "accuracy_without_rejection": metricas.get('acuracia_sem_rejeicao', 0),
        "accuracy_with_rejection": metricas.get('acuracia_com_rejeicao', 0),
        "rejection_rate": metricas.get('taxa_rejeicao_teste', 0)
        # Se você tiver "rejection_rate_total" calculado em algum lugar, adicione aqui
    },
    "explanation_stats": {
        "positive": metricas.get('stats_explicacao_positive', {}),
        "negative": metricas.get('stats_explicacao_negative', {}),
        "rejected": metricas.get('stats_explicacao_rejected', {})
    },
    "computation_time": {
        "total": metricas.get('tempo_total', 0),
        "mean_per_instance": metricas.get('tempo_medio_instancia', 0),
        "positive": metricas.get('tempo_medio_positivas', 0),
        "negative": metricas.get('tempo_medio_negativas', 0),
        "rejected": metricas.get('tempo_medio_rejeitadas', 0)
    },
    # Convertendo a lista de tuplas para uma lista de dicionários
    "top_features": [
        {"feature": name, "count": count} for name, count in metricas.get('features_frequentes', [])
    ]
}

# 2. Chamar a função para atualizar o arquivo JSON (usando a chamada de 3 argumentos que funcionou)
try:
    ...
    print(f"Resultados para '{dataset_key}' salvos com sucesso no JSON.")
except Exception as e:
    print(f"ERRO ao salvar os resultados no JSON: {e}")

# --- FIM DO BLOCO NOVO E COMPLETO ---