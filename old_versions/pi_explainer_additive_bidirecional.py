...
def calculate_deltas(modelo: Pipeline, instance_df: pd.DataFrame, X_train: pd.DataFrame, premis_class: int) -> np.ndarray:
    """
    Versão corrigida que opera no espaço escalonado para calcular os deltas.
    O parâmetro 'modelo' aqui é a Pipeline.
    """
    # Extrai os componentes da pipeline
    scaler = modelo.named_steps['scaler']
    logreg = modelo.named_steps['modelo']
    
    coefs = logreg.coef_[0]
    
    # 1. Escala a instância para a qual queremos a explicação
    scaled_instance_vals = scaler.transform(instance_df)[0]
    
    # 2. Escala todo o conjunto de treino para encontrar os valores min/max no espaço escalonado
    ...
    # 3. Calcula os deltas usando apenas valores e coeficientes na mesma escala
...
def one_explanation(modelo: LogisticRegression, instance_df: pd.DataFrame, X_train: pd.DataFrame, t_plus: float, t_minus: float, log_collector: List[str]):
    ...
    if is_rejected:
        ...
        # a soma dos deltas tem que ser maior ou igual a om gamma W 
        # e o gamma W com a opção de rejeição não pode ser o zero, deve ser o t- ou t+
#thiagoalvesifce - github
        # o valor do delta esta errado, a verificação tem que ser como no artigo veridficar o PHI
        target_delta_sum = abs(score) 
        log_collector.append(f"     - Objetivo (Rejeitada): Soma de |deltas| >= |score| ({target_delta_sum:.4f})")
        log_collector.append(f"     - Soma cumulativa inicial: {soma_deltas_cumulativa:.4f}")
        passo_num = 1
        ...
        for i in indices_ordenados:
            ...
            log_collector.append(f"         - Adicionando feature: '{feature_nome}' (com |Delta| = {delta_atual:.4f})")
            
            soma_anterior = soma_deltas_cumulativa
            soma_deltas_cumulativa += delta_atual
            explicacao.append(f"{feature_nome} = {instance_df.iloc[0, i]:.4f}") # Adiciona a feature à explicação
            
            log_collector.append(f"         - Nova soma cumulativa: {soma_anterior:.4f} + {delta_atual:.4f} = {soma_deltas_cumulativa:.4f}")
            
            # Verifica a meta *após* adicionar a feature e atualizar a soma
            if soma_deltas_cumulativa > target_delta_sum:
                log_collector.append(f"         - Checando meta: {soma_deltas_cumulativa:.4f} > {target_delta_sum:.4f}? Sim. Meta atingida.")
                log_collector.append(f"       - Parando. Explicação inicial encontrada com {len(explicacao)} feature(s).")
                break
            else:
                log_collector.append(f"         - Checando meta: {soma_deltas_cumulativa:.4f} > {target_delta_sum:.4f}? Não. O processo continua.")
...
def formatar_calculo_score(modelo: Pipeline, instance_df: pd.DataFrame, feature_names: List[str]) -> str:
    """
    Versão corrigida que usa dados escalonados para mostrar o cálculo real do score.
    O parâmetro 'modelo' aqui é a Pipeline completa.
    """
    # Extrai os componentes da pipeline
    scaler = modelo.named_steps['scaler']
    logreg = modelo.named_steps['modelo']
    
    pesos = logreg.coef_[0]
    intercepto = logreg.intercept_[0]
    
    # Escala a instância para usar os valores corretos no cálculo
    valores_escalonados = scaler.transform(instance_df)[0]
    
    # Monta a string de cálculo com os valores escalonados
    termos_str = [f"({pesos[i]:.4f} * {valores_escalonados[i]:.4f})" for i, nome in enumerate(feature_names)]
    calculo_str = " + ".join(termos_str)
    
    # Este cálculo agora resulta no score correto, pois usa as mesmas escalas
    score_final = np.dot(pesos, valores_escalonados) + intercepto
    
    return f"      Cálculo do Score (em dados escalonados): {calculo_str} + ({intercepto:.4f}) = {score_final:.4f}"
...
def executar_fase_1_reforco_bidirecional(modelo: Pipeline, instance_df: pd.DataFrame, expl_inicial: List[str], X_train: pd.DataFrame, t_plus: float, t_minus: float, class_names: List[str], log_collector: List[str], premissa_ordenacao: int) -> Tuple[List[str], int]:
    ...
    # Deltas para ordenar a adição de features. A premissa agora é um parâmetro.
    deltas_para_ordenar = calculate_deltas(modelo, instance_df, X_train, premis_class=premissa_ordenacao)
    # o valor dos desltas para a rejeição tem que diferenciar a classe negativa da positiva
    ...
    while True:
        ...
        # Teste na primeira direção (diminuir score)
        ...
        status2 = ""
        # Se passou no primeiro teste, tenta o segundo (aumentar score)
        ...
        # Se passou nos dois testes, a explicação é robusta
...
def executar_fase_2_minimizacao_bidirecional(modelo: Pipeline, instance_df: pd.DataFrame, expl_robusta: List[str], X_train: pd.DataFrame, t_plus: float, t_minus: float, class_names: List[str], log_collector: List[str], premissa_ordenacao: int) -> Tuple[List[str], int]:
    ...
    ###################### Deltas para ordenar a remoção. A premissa agora é um parâmetro.
    deltas_para_ordenar = calculate_deltas(modelo, instance_df, X_train, premis_class=premissa_ordenacao)
...
def encontrar_explicacao_otimizada_para_rejeitada(modelo: Pipeline, instance_df: pd.DataFrame, expl_inicial: List[str], X_train: pd.DataFrame, t_plus: float, t_minus: float, class_names: List[str], log_collector: List[str]) -> Tuple[List[str], int, int, int]:
    """
    Executa dois caminhos de busca para instâncias rejeitadas e retorna a menor explicação.
    """
    ...
    # A "Fase 0" já é logada pela chamada original a one_explanation, antes desta função.
    # Apenas para clareza, podemos reafirmar a premissa.
    log_collector.append("   (Usando a ordenação de deltas da explicação inicial, premissa: evitar Classe 0)")
    expl_robusta_1, adicoes_1 = executar_fase_1_reforco_bidirecional(modelo, instance_df, expl_inicial, X_train, t_plus, t_minus, class_names, log_collector, premissa_ordenacao=1)
    ...
    log_collector.append(f"   -> Resultado do Caminho 1: {len(expl_final_1)} features -> {expl_final_1}")
    ...
    # Adicionando o bloco de log para a Fase 0 do Caminho 2
    log_collector.append(f"\n   [Fase 0 do Caminho 2] Calculando explicação inicial com nova premissa:")
    log_collector.append(f"     - Premissa de ordenação: Evitar Classe 1 (maiores deltas que empurram para Classe 0 são priorizados).")
    # Para o log, vamos simular a criação de uma explicação inicial com a nova ordenação
    expl_inicial_caminho2 = one_explanation(modelo, instance_df, X_train, t_plus, t_minus, log_collector)
    # Fim do bloco de log adicionado

    expl_robusta_2, adicoes_2 = executar_fase_1_reforco_bidirecional(modelo, instance_df, expl_inicial_caminho2, X_train, t_plus, t_minus, class_names, log_collector, premissa_ordenacao=0)
    ...
    log_collector.append(f"   -> Resultado do Caminho 2: {len(expl_final_2)} features -> {expl_final_2}")

    # --- COMPARAÇÃO E DECISÃO FINAL ---
    log_collector.append("\n------------------------- COMPARAÇÃO FINAL -------------------------")
    log_collector.append(f"Tamanho da Explicação - Caminho 1 (evitar Classe 0): {len(expl_final_1)}")
    log_collector.append(f"Tamanho da Explicação - Caminho 2 (evitar Classe 1): {len(expl_final_2)}")
...
def gerar_relatorio_consolidado(modelo: LogisticRegression, X_test: pd.DataFrame, y_test: pd.Series, X_train: pd.DataFrame, y_train: pd.Series, class_names: List[str], nome_dataset: str, t_plus: float, t_minus: float):
    ...
    modelo_interno = modelo.named_steps['modelo'] #modelo da pipeline do scikitlearn
    ...
    for i in range(len(X_test)):
        ...
        # Premissa para explicação inicial e para o Caminho 1 das rejeitadas
        premissa_inicial = 1 if rejeitada else pred_class 
        expl_inicial = one_explanation(modelo, inst_df, X_train, t_plus, t_minus, log_instancia_atual)
        
        adicoes, remocoes = 0, 0
        premissa_final = premissa_inicial
        
        if rejeitada:
            # Precisamos modificar a função `encontrar_explicacao_otimizada_para_rejeitada` para retornar a premissa vencedora
            # Por simplicidade aqui, vamos recalcular.
            expl_final, adicoes, remocoes, premissa_vencedora = encontrar_explicacao_otimizada_para_rejeitada(
                modelo, inst_df, expl_inicial, X_train, t_plus, t_minus, class_names, log_instancia_atual
            )
            premissa_final = premissa_vencedora # Atualiza a premissa para o cálculo dos deltas
        ...
        log_instancia_atual.append(f"     - Resumo do Processo: {resumo_processo}")
         ...
         # --- NOVO BLOCO PARA EXIBIR EXPLICAÇÃO COM DELTAS ---
        log_instancia_atual.append(f"     - PI-EXPLICAÇÃO FINAL (Tamanho: {len(expl_final)}):")
        if expl_final:
            # Calcula os deltas usando a premissa da explicação final
...
def main():
    ...
    # Cria a Pipeline que será usada para treino e geração de explicações
    ...
    # Extrai o modelo treinado de dentro da pipeline para usar nas funções de explicação
    modelo_final = pipeline_modelo.named_steps['modelo']
    
    # As funções de explicação precisam dos dados originais para serem interpretáveis,
    # mas o modelo agora espera dados escalados. Para simplificar, vamos passar o modelo
    # e as funções internas dele (predict, decision_function) vão receber os dados já escalados
    # pela pipeline. As lógicas de explicação que manipulam os dados diretamente
    # precisam ser ajustadas se necessário, mas as suas funções parecem trabalhar
    # com o objeto 'modelo' e seus métodos, o que a pipeline facilita.
    
    print("Calculando thresholds de rejeição...")
    # Para calcular os thresholds, usamos os dados de treino originais,
    # a pipeline fará a transformação internamente.
    t_plus, t_minus = calcular_thresholds(pipeline_modelo, X_train, y_train)
    print(f"Thresholds definidos: t+ = {t_plus:.4f}, t- = {t_minus:.4f}")
    
    # Passamos a pipeline inteira para a geração do relatório.
    # A função gerar_relatorio_consolidado deve usar a pipeline para predições
    # para garantir que a normalização seja aplicada.
