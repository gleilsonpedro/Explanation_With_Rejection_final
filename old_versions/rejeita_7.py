...
def gerar_explicacao_completa_para_rejeicao(
    ...
    """
    Implementa a lógica de robustez e minimização bi-direcional para instâncias rejeitadas.
    Uma explicação é robusta se mantiver a instância na zona de rejeição quando
    perturbada para o pior caso positivo (diminuindo o score) E para o pior caso
    negativo (aumentando o score).
    """
    logs_processo = []

    # --- FASE 1: REFORÇO ADITIVO BI-DIRECIONAL ---
    logs_processo.append("\nFASE 1: Reforço Aditivo para Robustez BI-DIRECIONAL")
    ...
    iter_count = 0
    while iter_count < len(nomes_features_todas_ordenadas) + 1:
        iter_count += 1
        
        _, status_pos, pred_str_pos = perturbar_features(modelo, instancia_df, expl_robusta, X_train, t_plus, t_minus, class_names_binario, direcao_perturbacao_override=1)
        _, status_neg, pred_str_neg = perturbar_features(modelo, instancia_df, expl_robusta, X_train, t_plus, t_minus, class_names_binario, direcao_perturbacao_override=0)
        
        eh_robusta_pos = status_pos.startswith("VÁLIDA (REJEIÇÃO")
        eh_robusta_neg = status_neg.startswith("VÁLIDA (REJEIÇÃO")

        logs_processo.append(f"  - Verificando robustez da explicação com {len(expl_robusta)} feats:")
        logs_processo.append(f"    - Perturbando para ↓ score: Status '{status_pos}', Pred: {pred_str_pos}")
        logs_processo.append(f"    - Perturbando para ↑ score: Status '{status_neg}', Pred: {pred_str_neg}")

        if eh_robusta_pos and eh_robusta_neg:
            logs_processo.append("    => Explicação é BI-DIRECIONALMENTE ROBUSTA. Prosseguindo para minimização.")
            break
            
        if len(expl_robusta) == X_train.shape[1]:
            logs_processo.append("    => ATENÇÃO: Todas as features foram adicionadas, mas a explicação ainda não é bi-direcionalmente robusta.")
            break

        logs_processo.append("    => Explicação NÃO é robusta. Adicionando feature...")
        features_ja_na_explicacao = set([fs.split(' = ')[0] for fs in expl_robusta])
        feature_adicionada = False
        for nome_feature_a_adicionar in nomes_features_todas_ordenadas:
            if nome_feature_a_adicionar not in features_ja_na_explicacao:
                ...
                expl_robusta.append(f"{nome_feature_a_adicionar} = {valor_feature_a_adicionar:.4f}")
                logs_processo.append(f"      - Feature '{nome_feature_a_adicionar}' adicionada.")
                feature_adicionada = True
                break
        
        if not feature_adicionada: break

    status_final_fase1 = "VÁLIDA (REJEIÇÃO MANTEVE-SE REJEITADA)" if (eh_robusta_pos and eh_robusta_neg) else "INVÁLIDA (NÃO ROBUSTA BI-DIRECIONALMENTE)"
    ...
    if not status_final_fase1.startswith("VÁLIDA"):
        logs_processo.append("  Explicação de entrada não é robusta, pulando minimização.")
        return expl_final_min, status_final_fase1, logs_processo

    map_nome_feature_para_strcompleta = {feat_str.split(' = ')[0]: feat_str for feat_str in expl_final_min}
    features_ordenadas_para_remocao = sorted(
        list(map_nome_feature_para_strcompleta.keys()),
        key=lambda nome_feat: abs(todos_deltas_instancia[X_train.columns.get_loc(nome_feat)])
    )
    ...
    for nome_feature_tentar_remover in features_ordenadas_para_remocao:
        ...
        if nome_feature_tentar_remover not in nomes_atuais_expl_min: continue
            
        expl_temporaria = [map_nome_feature_para_strcompleta[nome_f] for nome_f in nomes_atuais_expl_min if nome_f != nome_feature_tentar_remover]
        
        logs_processo.append(f"  - Tentando remover '{nome_feature_tentar_remover}'...")
        ...
        eh_robusta_pos_min = status_pos_min.startswith("VÁLIDA (REJEIÇÃO")
        eh_robusta_neg_min = status_neg_min.startswith("VÁLIDA (REJEIÇÃO")

        if eh_robusta_pos_min and eh_robusta_neg_min:
            logs_processo.append(f"    => REMOÇÃO BEM-SUCEDIDA. Explicação com {len(expl_temporaria)} feats ainda é bi-direcionalmente robusta.")
            expl_final_min = expl_temporaria
        else:
            logs_processo.append(f"    => FALHA NA REMOÇÃO. Explicação tornou-se inválida.")

    status_final_minimizacao = status_final_fase1
    
    return expl_final_min, status_final_minimizacao, logs_processo
...
def gerar_relatorio_detalhado_por_instancia(
    ...
    Para instâncias rejeitadas, usa a lógica de robustez bi-direcional.
    ...
    caminho_arquivo_relatorio = os.path.join(diretorio_relatorios, f"rel_deta_7 {nome_dataset_original}.txt")
    ...
    with open(caminho_arquivo_relatorio, "w", encoding="utf-8") as f:
        ...
        for i in range(len(X_test)):
            ...
            if original_foi_rejeitada:
                # Usa a nova lógica bi-direcional para instâncias rejeitadas
                expl_final, status_final, logs_rejeicao = gerar_explicacao_completa_para_rejeicao(
                    modelo, instancia_atual_df, expl_f0, X_train, t_plus, t_minus, class_names_binario
                )
                for log_line in logs_rejeicao: f.write(f"{log_line}\n")
                f.write("\n")
            else:
                # Usa a lógica antiga para instâncias classificadas
                # --- FASE 1 ---
                expl_f1, status_f1, inst_pert_f1, log_f1, _ = executar_fase_1_refinamento_robustez(
                    modelo, instancia_atual_df, expl_f0, X_train, t_plus, t_minus, class_names_binario
                )
                ...
                # --- FASE 2 ---
                expl_final, status_final, _, log_f2, _ = executar_fase_2_minimizacao_minimalidade(
                    modelo, instancia_atual_df, expl_f1, X_train, t_plus, t_minus, class_names_binario, 
                    status_f1, inst_pert_f1 
                )
            ...
            # Reavalia o status e a predição para consistência no log final
