...
Para instâncias rejeitadas, o script testa duas direções de perturbação
(pior caso positivo e pior caso negativo) e seleciona a que resulta na
explicação mais minimalista que mantém a rejeição.
...
def one_explanation(modelo: LogisticRegression,
                    ...
                    t_minus: Optional[float]) -> List[str]:
    ...
    if foi_rejeitada_originalmente:
        ...
        if pred_class_original == 1:
            ...
            limiar_alvo_score = effective_floor_class1

            phi_target_sum_contrib = limiar_alvo_score - score_base + EPSILON
            ...
            limiar_alvo_score = effective_ceiling_class0

            phi_target_sum_abs_deltas = score_base - limiar_alvo_score + EPSILON
            soma_cumulativa_abs_deltas = 0.0
...
def perturbar_features(modelo: LogisticRegression, instance_original_df: pd.DataFrame,
                       ...
                       direcao_perturbacao_override: Optional[int] = None
    ...
    if existem_features_para_perturbar:
        perturbar_simulando_classe_1 = False
        if original_foi_rejeitada and direcao_perturbacao_override is not None:
            perturbar_simulando_classe_1 = (direcao_perturbacao_override == 1)
        elif pred_classe_original_idx == 1:
            perturbar_simulando_classe_1 = True
        ...
        for feat_nome_perturbar in features_para_perturbar:
            try:
                ...
                if perturbar_simulando_classe_1:
...
def gerar_explicacao_rejeicao_com_prioridade(
    modelo: LogisticRegression,
    instancia_df: pd.DataFrame,
    expl_inicial: List[str],
    X_train: pd.DataFrame,
    t_plus: float,
    t_minus: float,
    class_names_binario: List[str],
    prioridade_perturbacao: int  # 1 para priorizar positivo, 0 para priorizar negativo
) -> Tuple[List[str], str, List[str]]:
    """
    Gera uma explicação bi-direcionalmente robusta e mínima para uma instância rejeitada,
    seguindo uma ordem de prioridade para os testes de perturbação.
    """
    logs_processo = []
    
    # Define a ordem de teste baseada na prioridade
    direcoes = [prioridade_perturbacao, 1 - prioridade_perturbacao]
    
    # --- FASE 1: REFORÇO ADITIVO BI-DIRECIONAL ---
    logs_processo.append(f"\nFASE 1: Reforço Aditivo (Prioridade: perturbar para classe {prioridade_perturbacao})")
    ...
    while True:
        status_geral = "ROBUSTA"
        # Loop de direções: testa a primeira prioridade, depois a segunda
        for direcao_teste in direcoes:
            _, status, _ = perturbar_features(modelo, instancia_df, expl_robusta, X_train, t_plus, t_minus, class_names_binario, direcao_perturbacao_override=direcao_teste)
            if not status.startswith("VÁLIDA (REJEIÇÃO"):
                status_geral = "NAO_ROBUSTA"
                break # Se falhar em uma direção, não precisa testar a outra
        
        if status_geral == "ROBUSTA":
            logs_processo.append(f"  - Explicação com {len(expl_robusta)} feats é BI-DIRECIONALMENTE ROBUSTA.")
            break

        if len(expl_robusta) == X_train.shape[1]:
            logs_processo.append("  - ATENÇÃO: Todas as features adicionadas, mas a explicação ainda não é robusta.")
            break
            
        logs_processo.append(f"  - Explicação com {len(expl_robusta)} feats NÃO é robusta. Adicionando feature...")
        features_ja_na_explicacao = {fs.split(' = ')[0] for fs in expl_robusta}
        adicionou_feature = False
        for nome_feature in nomes_features_todas_ordenadas:
            if nome_feature not in features_ja_na_explicacao:
                valor_feature = instancia_df.iloc[0][nome_feature]
                expl_robusta.append(f"{nome_feature} = {valor_feature:.4f}")
                logs_processo.append(f"    - Feature '{nome_feature}' adicionada.")
                adicionou_feature = True
                break
        if not adicionou_feature: break

    status_final_fase1 = "VÁLIDA (REJEIÇÃO MANTEVE-SE REJEITADA)" if status_geral == "ROBUSTA" else "INVÁLIDA"
    logs_processo.append(f"\n  Explicação final da Fase 1 ({len(expl_robusta)} feats), Status: {status_final_fase1}\n")

    # --- FASE 2: MINIMIZAÇÃO SUBTRATIVA BI-DIRECIONAL ---
    logs_processo.append("FASE 2: Simplificação BI-DIRECIONAL da Explicação Robusta")
    expl_final_min = list(expl_robusta)
    
    if not status_final_fase1.startswith("VÁLIDA"):
        return expl_final_min, status_final_fase1, logs_processo

    map_nome_para_str = {fs.split(' = ')[0]: fs for fs in expl_final_min}
    features_para_remover = sorted(list(map_nome_para_str.keys()), key=lambda nome: abs(todos_deltas_instancia[X_train.columns.get_loc(nome)]))
    
    for feature_a_remover in features_para_remover:
        if len(expl_final_min) <= 1: break
        
        expl_temporaria = [v for k, v in map_nome_para_str.items() if k != feature_a_remover]
        
        # Testa a robustez bi-direcional da explicação temporária
        _, status_pos_min, _ = perturbar_features(modelo, instancia_df, expl_temporaria, X_train, t_plus, t_minus, class_names_binario, direcao_perturbacao_override=1)
        _, status_neg_min, _ = perturbar_features(modelo, instancia_df, expl_temporaria, X_train, t_plus, t_minus, class_names_binario, direcao_perturbacao_override=0)

        if status_pos_min.startswith("VÁLIDA (REJEIÇÃO") and status_neg_min.startswith("VÁLIDA (REJEIÇÃO"):
            logs_processo.append(f"  - REMOÇÃO BEM-SUCEDIDA de '{feature_a_remover}'.")
            expl_final_min = expl_temporaria
            map_nome_para_str.pop(feature_a_remover)
        else:
            logs_processo.append(f"  - FALHA NA REMOÇÃO de '{feature_a_remover}'.")
...
def executar_fase_0_explicacao_inicial(
    ...
    log_detalhes_fase_0.append(f"  Explicação Inicial ({len(expl_inicial)} features):")
    if expl_inicial:
...
def executar_fase_1_refinamento_robustez(
    modelo: LogisticRegression,
    instancia_df: pd.DataFrame,
    explicacao_candidata_fase0: List[str],
    X_train: pd.DataFrame,
    t_plus: Optional[float],
    t_minus: Optional[float],
    class_names_binario: List[str],
    direcao_perturbacao_override: Optional[int] = None # NOVO PARÂMETRO
    ...
    """
    log_detalhes_fase_1 = ["FASE 1: Teste de Robustez e Reforço da Explicação"]

    expl_robusta = list(explicacao_candidata_fase0)
    ...
    inst_pert_robusta, status_robusto, pred_str_log_inicial = perturbar_features(
        ...
        direcao_perturbacao_override=direcao_perturbacao_override # PASSANDO O PARÂMETRO
    )
    ...
    pred_str_log_apos_adicao = pred_str_log_inicial
    ...
    if precisa_refinar_aditivamente:
        ...
        for nome_feature_a_adicionar in nomes_features_todas_ordenadas:
            if nome_feature_a_adicionar not in features_ja_na_explicacao:
                ...
                status_robusto = status_refinado_loop
                inst_pert_robusta = inst_pert_refinada_loop
                pred_str_log_apos_adicao = pred_str_log_loop
...
def executar_fase_2_minimizacao_minimalidade(
    modelo: LogisticRegression,
    instancia_df: pd.DataFrame,
    explicacao_candidata_robusta: List[str],
    X_train: pd.DataFrame,
    t_plus: Optional[float],
    t_minus: Optional[float],
    class_names_binario: List[str],
    status_entrada_fase2: str,
    inst_pert_entrada_fase2: pd.DataFrame, # Não é mais usada aqui, mas mantida por compatibilidade
    direcao_perturbacao_override: Optional[int] = None # NOVO PARÂMETRO
) -> Tuple[List[str], str, pd.DataFrame, List[str], int]:
    """
    Fase 2: Tenta simplificar (minimizar) a explicação robusta da Fase 1, mantendo a robustez.
    """
    log_detalhes_fase_2 = ["FASE 2: Simplificação da Explicação Robusta (Remoção Subtrativa)"]

    expl_final_min = list(explicacao_candidata_robusta)
    status_final_min = status_entrada_fase2
    # inst_pert_final_min = inst_pert_entrada_fase2.copy() # inst_pert é reavaliada a cada passo

    # Reavalia a instância perturbada e a predição da explicação que entra na Fase 2
    # com a direção de perturbação correta, se aplicável
    temp_inst_pert_entrada, _, pred_str_log_entrada_f2 = perturbar_features(
        modelo, instancia_df, expl_final_min, X_train, t_plus, t_minus, class_names_binario,
        direcao_perturbacao_override=direcao_perturbacao_override # PASSANDO O PARÂMETRO
    )
    inst_pert_final_min = temp_inst_pert_entrada.copy()
    ...
    if deve_minimizar:
        ...
        features_ordenadas_para_remocao = sorted(
            ...
            key=lambda nome_feat: abs(deltas_features_na_explicacao[nome_feat])
        )
        ...
        for nome_feature_tentar_remover in features_ordenadas_para_remocao:
            ...
            if not expl_temporaria_formatada and X_train.shape[1] > 0 :
                 ...
                 status_tentativa_min = "INVÁLIDA (EXPLICAÇÃO VAZIA)"
                 pred_str_log_tentativa = "N/A (Explicação vazia)"
                 inst_pert_tentativa_min = instancia_df.copy() # Placeholder
            else:
            ...
            if status_tentativa_min.startswith("VÁLIDA"):
                ...
                melhor_status_minimo_valido = status_final_min
                melhor_inst_pert_minima_valida = inst_pert_final_min.copy()
                pred_str_log_melhor_minima = pred_str_log_tentativa
            else:
        ...
        pred_str_log_final_da_fase2 = pred_str_log_melhor_minima
        ...
        pred_str_log_final_da_fase2 = pred_str_log_entrada_f2
...
def gerar_relatorio_resumido(
    ...
    contagem_vencedor_rejeicao = Counter() # NOVO: para contar qual direção de perturbação venceu
    ...
    for i in range(len(X_test)):
        ...
        expl_final_para_estatisticas = []
        status_final_para_estatisticas = ""
        features_add_f1_para_estatisticas = 0
        features_rem_f2_para_estatisticas = 0

        expl_f0, _ = executar_fase_0_explicacao_inicial(modelo, instancia_atual_df, X_train, t_plus, t_minus)

        if original_foi_rejeitada:
            # Tentativa 1: Pior Caso simulando Classe Positiva (direcao_perturbacao_override=1)
            expl_f1_pos, status_f1_pos, inst_pert_f1_pos, _, f_add_f1_pos = executar_fase_1_refinamento_robustez(
                modelo, instancia_atual_df, expl_f0, X_train, t_plus, t_minus, class_names_binario, direcao_perturbacao_override=1
            )
            expl_final_pos, status_final_pos, _, _, f_rem_f2_pos = executar_fase_2_minimizacao_minimalidade(
                modelo, instancia_atual_df, expl_f1_pos, X_train, t_plus, t_minus, class_names_binario,
                status_f1_pos, inst_pert_f1_pos, direcao_perturbacao_override=1
            )
            len_expl_pos = len(expl_final_pos) if status_final_pos.startswith("VÁLIDA (REJEIÇÃO") else float('inf')

            # Tentativa 2: Pior Caso simulando Classe Negativa (direcao_perturbacao_override=0)
            expl_f1_neg, status_f1_neg, inst_pert_f1_neg, _, f_add_f1_neg = executar_fase_1_refinamento_robustez(
                modelo, instancia_atual_df, expl_f0, X_train, t_plus, t_minus, class_names_binario, direcao_perturbacao_override=0
            )
            expl_final_neg, status_final_neg, _, _, f_rem_f2_neg = executar_fase_2_minimizacao_minimalidade(
                modelo, instancia_atual_df, expl_f1_neg, X_train, t_plus, t_minus, class_names_binario,
                status_f1_neg, inst_pert_f1_neg, direcao_perturbacao_override=0
            )
            len_expl_neg = len(expl_final_neg) if status_final_neg.startswith("VÁLIDA (REJEIÇÃO") else float('inf')

            if len_expl_pos <= len_expl_neg and len_expl_pos != float('inf'):
                expl_final_para_estatisticas = expl_final_pos
                status_final_para_estatisticas = status_final_pos
                features_add_f1_para_estatisticas = f_add_f1_pos
                features_rem_f2_para_estatisticas = f_rem_f2_pos
                contagem_vencedor_rejeicao["Pior Caso Positivo"] += 1
            elif len_expl_neg < len_expl_pos and len_expl_neg != float('inf'):
                expl_final_para_estatisticas = expl_final_neg
                status_final_para_estatisticas = status_final_neg
                features_add_f1_para_estatisticas = f_add_f1_neg
                features_rem_f2_para_estatisticas = f_rem_f2_neg
                contagem_vencedor_rejeicao["Pior Caso Negativo"] += 1
            elif len_expl_pos == float('inf') and len_expl_neg != float('inf'): # Apenas Negativo foi válido
                expl_final_para_estatisticas = expl_final_neg
                status_final_para_estatisticas = status_final_neg
                features_add_f1_para_estatisticas = f_add_f1_neg
                features_rem_f2_para_estatisticas = f_rem_f2_neg
                contagem_vencedor_rejeicao["Pior Caso Negativo (Positivo Inválido)"] +=1
            elif len_expl_neg == float('inf') and len_expl_pos != float('inf'): # Apenas Positivo foi válido
                expl_final_para_estatisticas = expl_final_pos
                status_final_para_estatisticas = status_final_pos
                features_add_f1_para_estatisticas = f_add_f1_pos
                features_rem_f2_para_estatisticas = f_rem_f2_pos
                contagem_vencedor_rejeicao["Pior Caso Positivo (Negativo Inválido)"] +=1
            else: # Ambas inválidas
                expl_final_para_estatisticas = expl_final_pos # ou neg, tanto faz
                status_final_para_estatisticas = status_final_pos # ou neg
                features_add_f1_para_estatisticas = f_add_f1_pos
                features_rem_f2_para_estatisticas = f_rem_f2_pos
                contagem_vencedor_rejeicao["Ambas Inválidas"] +=1
        else: # Instância não foi rejeitada, lógica original
            expl_f1, status_f1, inst_pert_f1, _, f_add_f1 = executar_fase_1_refinamento_robustez(
                modelo, instancia_atual_df, expl_f0, X_train, t_plus, t_minus, class_names_binario, direcao_perturbacao_override=None
            )
            expl_final, status_final, _, _, f_rem_f2 = executar_fase_2_minimizacao_minimalidade(
                modelo, instancia_atual_df, expl_f1, X_train, t_plus, t_minus, class_names_binario,
                status_f1, inst_pert_f1, direcao_perturbacao_override=None
            )
            expl_final_para_estatisticas = expl_final
            status_final_para_estatisticas = status_final
            features_add_f1_para_estatisticas = f_add_f1
            features_rem_f2_para_estatisticas = f_rem_f2

        if features_add_f1_para_estatisticas > 0:
            total_features_adicionadas_fase1 += features_add_f1_para_estatisticas
            instancias_com_adicao_fase1 += 1
        if features_rem_f2_para_estatisticas > 0:
            total_features_removidas_fase2 += features_rem_f2_para_estatisticas
            instancias_com_remocao_fase2 += 1

        contagem_validacao_final[status_final_para_estatisticas] += 1
        if status_final_para_estatisticas.startswith("VÁLIDA"):
            lista_tamanhos_explicacoes_validas.append(len(expl_final_para_estatisticas))
            if expl_final_para_estatisticas:
                for feat_val_str in expl_final_para_estatisticas:
    ...
    with open(caminho_arquivo_relatorio, "w", encoding="utf-8") as f:
        ...
        if t_plus is not None and t_minus is not None:
            ...
            total_rejeitadas_processadas = sum(contagem_vencedor_rejeicao.values())
            if total_rejeitadas_processadas > 0:
                for tipo_vencedor, contagem in contagem_vencedor_rejeicao.items():
                    perc = (contagem / total_rejeitadas_processadas) * 100
                    f.write(f"  - {tipo_vencedor}: {contagem} ({perc:.2f}% das rejeitadas com explicação)\n")
            else:
                f.write("  - Nenhuma instância rejeitada processada com a dupla abordagem ou nenhuma teve explicação válida.\n")
        ...
        f.write("\nLembretes sobre os Status de Validação no Relatório Final:\n"
                ...
                "  - INVÁLIDA (REJEIÇÃO SAIU DA ZONA DE REJEIÇÃO): Instância originalmente rejeitada saiu da zona de rejeição após perturbação.\n"
                "  - INVÁLIDA (CLASSIFICADA MUDOU DE CLASSE...): Instância classificada mudou de classe após perturbação.\n"
...
def gerar_relatorio_detalhado_por_instancia(
    ...
    t_plus: Optional[float],
    t_minus: Optional[float],
    wr_cost_usado: Optional[float] = None
) -> str:
    """
    Gera um relatório detalhado mostrando cada passo da execução por instância.
    Para instâncias rejeitadas, usa a lógica de robustez bi-direcional com dupla prioridade.
    Para instâncias classificadas, usa a lógica de robustez uni-direcional.
    """
    diretorio_relatorios = "relatorios_refatorados"
    os.makedirs(diretorio_relatorios, exist_ok=True)
    caminho_arquivo_relatorio = os.path.join(diretorio_relatorios, f"rel_det_8_{nome_dataset_original}.txt")
    ...
    with open(caminho_arquivo_relatorio, "w", encoding="utf-8") as f:
        ...
        for i in range(len(X_test)):
            ...
            expl_f0, log_f0 = executar_fase_0_explicacao_inicial(modelo, instancia_atual_df, X_train, t_plus, t_minus)
            for log_line in log_f0: f.write(f"{log_line}\n")
            
            if original_foi_rejeitada:
                f.write("\nAVALIAÇÃO DE ROBUSTEZ BI-DIRECIONAL PARA INSTÂNCIA REJEITADA (ESTRATÉGIA DE DUAS CORRIDAS)\n")
    
                # --- Corrida 1: Prioridade Positiva ---
                f.write("\n--- TENTATIVA 1: Prioridade na perturbação para CLASSE POSITIVA (diminuir score) ---\n")
                expl_pos, status_pos, logs_pos = gerar_explicacao_rejeicao_com_prioridade(
                    modelo, instancia_atual_df, expl_f0, X_train, t_plus, t_minus, class_names_binario, prioridade_perturbacao=1
                )
                for log_line in logs_pos: f.write(f"{log_line}\n")
                len_pos = len(expl_pos) if status_pos.startswith("VÁLIDA") else float('inf')
                f.write(f"--- Fim Tentativa 1: Tamanho Expl = {'Inválida' if len_pos == float('inf') else len_pos} ---\n")

                # --- Corrida 2: Prioridade Negativa ---
                f.write("\n--- TENTATIVA 2: Prioridade na perturbação para CLASSE NEGATIVA (aumentar score) ---\n")
                expl_neg, status_neg, logs_neg = gerar_explicacao_rejeicao_com_prioridade(
                    modelo, instancia_atual_df, expl_f0, X_train, t_plus, t_minus, class_names_binario, prioridade_perturbacao=0
                )
                for log_line in logs_neg: f.write(f"{log_line}\n")
                len_neg = len(expl_neg) if status_neg.startswith("VÁLIDA") else float('inf')
                f.write(f"--- Fim Tentativa 2: Tamanho Expl = {'Inválida' if len_neg == float('inf') else len_neg} ---\n")
                
                # --- Decisão Final ---
                vencedor_str = ""
                if len_pos <= len_neg and len_pos != float('inf'):
                    expl_final = expl_pos
                    status_final = status_pos
                    vencedor_str = "Prioridade Positiva"
                    if len_pos == len_neg: vencedor_str += " (empate)"
                elif len_neg < len_pos and len_neg != float('inf'):
                    expl_final = expl_neg
                    status_final = status_neg
                    vencedor_str = "Prioridade Negativa"
                elif len_neg == float('inf') and len_pos != float('inf'): # Apenas Positivo foi válido
                    expl_final = expl_pos
                    status_final = status_pos
                    vencedor_str = "Prioridade Positiva (única válida)"
                elif len_pos == float('inf') and len_neg != float('inf'): # Apenas Negativo foi válido
                    expl_final = expl_neg
                    status_final = status_neg
                    vencedor_str = "Prioridade Negativa (única válida)"
                else: # Ambas inválidas
                    expl_final = []
                    status_final = "INVÁLIDA (AMBAS AS ABORDAGENS FALHARAM)"
                    vencedor_str = "Nenhuma"
                    
                f.write(f"\nDECISÃO: A explicação mais curta e robusta foi encontrada com a abordagem '{vencedor_str}'.\n")

            else:
                # Lógica uni-direcional para instâncias CLASSIFICADAS
                f.write("\n")
                expl_f1, status_f1, inst_pert_f1, log_f1, _ = executar_fase_1_refinamento_robustez(modelo, instancia_atual_df, expl_f0, X_train, t_plus, t_minus, class_names_binario)
                for log_line in log_f1: f.write(f"{log_line}\n")
                
                expl_final, status_final, _, log_f2, _ = executar_fase_2_minimizacao_minimalidade(modelo, instancia_atual_df, expl_f1, X_train, t_plus, t_minus, class_names_binario, status_f1, pd.DataFrame())
                for log_line in log_f2: f.write(f"{log_line}\n")
            ...
            if not expl_final:
                if status_final.startswith("VÁLIDA"):
                    f.write("      - Explicação vazia (instância inerentemente robusta).\n")
                else:
                    f.write("      - Nenhuma explicação robusta encontrada.\n")
            else:
                for idx_f, fv_str in enumerate(expl_final):
                    f.write(f"      {idx_f+1}. {fv_str}\n")
            
            f.write(f"    Status da Validação Final: {status_final}\n")
            f.write("-" * 70 + "\n")
...
def plotar_desempenho_fases_explicacao(
    features_adicionadas_fase1_lista: List[int],
    features_removidas_fase2_lista: List[int],
...
def plotar_correlacao_tamanho_confianca(
    tamanhos_explicacoes_validas: List[int],
    distancias_ao_limiar: List[float],
    nome_dataset: str
):
...
def gerar_visualizacoes_completas(
    nome_dataset_original: str,
    lista_tamanhos_explicacoes_validas: List[int],
    lista_todas_features_explicacoes_validas: List[str],
    contagem_validacao_final: Counter,
    ...
    if features_adicionadas_f1_todas_instancias is not None and \
       ...
       total_instancias_teste is not None:
    ...
    if tamanhos_explicacoes_para_correlacao and distancias_ao_limiar_para_correlacao:
         plotar_correlacao_tamanho_confianca(
             tamanhos_explicacoes_para_correlacao,
             distancias_ao_limiar_para_correlacao,
             nome_dataset_original
         )
...
def main():
    ...
    caminho_detalhado = gerar_relatorio_detalhado_por_instancia(
        ...
        nomes_classes_binarias, nome_dataset_usado,
        threshold_plus, threshold_minus,
        wr_cost_usado=WR_REJECTION_COST
    )
    ...
    tamanhos_plot_para_correlacao = []
    distancias_ao_limiar_validas_plot = []

    if X_teste_df is not None and not X_teste_df.empty:
        for i in range(len(X_teste_df)):
            if (i + 1) % 25 == 0 or i == len(X_teste_df) -1 :
                print(f"  Coletando dados da instância {i+1}/{len(X_teste_df)} para plots...")
            instancia_atual_df = X_teste_df.iloc[[i]]
            score_orig_inst = scores_teste_para_plot[i] # Usa o score já calculado
            original_foi_rejeitada = (threshold_plus is not None and threshold_minus is not None and (threshold_minus <= score_orig_inst <= threshold_plus))

            expl_final_para_plots = []
            status_final_para_plots = ""
            features_add_f1_para_plots = 0
            ...
            if original_foi_rejeitada:
                # Tentativa 1: Pior Caso simulando Classe Positiva (direcao_perturbacao_override=1)
                expl_f1_pos, status_f1_pos, inst_pert_f1_pos, _, f_add_f1_pos = executar_fase_1_refinamento_robustez(
                    modelo_logreg, instancia_atual_df, expl_f0, X_treino_df, threshold_plus, threshold_minus, nomes_classes_binarias, direcao_perturbacao_override=1
                )
                expl_final_pos, status_final_pos, _, _, f_rem_f2_pos = executar_fase_2_minimizacao_minimalidade(
                    modelo_logreg, instancia_atual_df, expl_f1_pos, X_treino_df, threshold_plus, threshold_minus, nomes_classes_binarias,
                    status_f1_pos, inst_pert_f1_pos, direcao_perturbacao_override=1
                )
                len_expl_pos = len(expl_final_pos) if status_final_pos.startswith("VÁLIDA (REJEIÇÃO") else float('inf')
                ...
                # Tentativa 2: Pior Caso simulando Classe Negativa (direcao_perturbacao_override=0)
                expl_f1_neg, status_f1_neg, inst_pert_f1_neg, _, f_add_f1_neg = executar_fase_1_refinamento_robustez(
                    modelo_logreg, instancia_atual_df, expl_f0, X_treino_df, threshold_plus, threshold_minus, nomes_classes_binarias, direcao_perturbacao_override=0
                )
                expl_final_neg, status_final_neg, _, _, f_rem_f2_neg = executar_fase_2_minimizacao_minimalidade(
                    modelo_logreg, instancia_atual_df, expl_f1_neg, X_treino_df, threshold_plus, threshold_minus, nomes_classes_binarias,
                    status_f1_neg, inst_pert_f1_neg, direcao_perturbacao_override=0
                )
                len_expl_neg = len(expl_final_neg) if status_final_neg.startswith("VÁLIDA (REJEIÇÃO") else float('inf')

                if len_expl_pos <= len_expl_neg and len_expl_pos != float('inf'):
                    expl_final_para_plots = expl_final_pos
                    status_final_para_plots = status_final_pos
                    features_add_f1_para_plots = f_add_f1_pos
                    lista_features_removidas_f2_plot.append(f_rem_f2_pos)
                elif len_expl_neg < len_expl_pos and len_expl_neg != float('inf'):
                    expl_final_para_plots = expl_final_neg
                    status_final_para_plots = status_final_neg
                    features_add_f1_para_plots = f_add_f1_neg
                    lista_features_removidas_f2_plot.append(f_rem_f2_neg)
                elif len_expl_pos == float('inf') and len_expl_neg != float('inf'):
                    expl_final_para_plots = expl_final_neg
                    status_final_para_plots = status_final_neg
                    features_add_f1_para_plots = f_add_f1_neg
                    lista_features_removidas_f2_plot.append(f_rem_f2_neg)
                elif len_expl_neg == float('inf') and len_expl_pos != float('inf'):
                    expl_final_para_plots = expl_final_pos
                    status_final_para_plots = status_final_pos
                    features_add_f1_para_plots = f_add_f1_pos
                    lista_features_removidas_f2_plot.append(f_rem_f2_pos)
                    ...
                    features_add_f1_para_plots = f_add_f1_pos
                    lista_features_removidas_f2_plot.append(f_rem_f2_pos)
            else: # Instância não foi rejeitada
                expl_f1, status_f1, inst_pert_f1, _, f_add_f1 = executar_fase_1_refinamento_robustez(
                    modelo_logreg, instancia_atual_df, expl_f0, X_treino_df, threshold_plus, threshold_minus, nomes_classes_binarias, direcao_perturbacao_override=None
                )
                expl_final, status_final, _, _, f_rem_f2 = executar_fase_2_minimizacao_minimalidade(
                    modelo_logreg, instancia_atual_df, expl_f1, X_treino_df, threshold_plus, threshold_minus, nomes_classes_binarias,
                    status_f1, inst_pert_f1, direcao_perturbacao_override=None
                )
                expl_final_para_plots = expl_final
                status_final_para_plots = status_final
                features_add_f1_para_plots = f_add_f1
                lista_features_removidas_f2_plot.append(f_rem_f2)

            lista_features_adicionadas_f1_plot.append(features_add_f1_para_plots)
            contagem_validacao_plot[status_final_para_plots] +=1

            if status_final_para_plots.startswith("VÁLIDA"):
                tamanhos_plot_geral.append(len(expl_final_para_plots))
                if expl_final_para_plots:
                    for fv_str in expl_final_para_plots:
                        features_plot.append(fv_str.split(" = ")[0])

                # Coleta de dados para Plot 6 (Correlação) - A lógica original já foca em instâncias CLASSIFICADAS válidas
                # Se a instância foi rejeitada, ela não entra aqui para o plot de correlação
                # pois `originalmente_classificada` será False.
                pred_classe_inst_original = modelo_logreg.predict(instancia_atual_df)[0]
                originalmente_classificada_para_correlacao = True
                if threshold_plus is not None and threshold_minus is not None:
                    if threshold_minus <= score_orig_inst <= threshold_plus:
                        originalmente_classificada_para_correlacao = False

                if originalmente_classificada_para_correlacao and status_final_para_plots.startswith("VÁLIDA (CLASSIFICADA MANTEVE CLASSE"):
                    distancia_calculada = 0.0
                    if threshold_plus is not None and threshold_minus is not None:
                        if pred_classe_inst_original == 1 and score_orig_inst > threshold_plus:
                            distancia_calculada = score_orig_inst - threshold_plus
                        elif pred_classe_inst_original == 0 and score_orig_inst < threshold_minus:
                            distancia_calculada = threshold_minus - score_orig_inst
                    else:
                        distancia_calculada = abs(score_orig_inst)
                    distancias_ao_limiar_validas_plot.append(distancia_calculada)
                    tamanhos_plot_para_correlacao.append(len(expl_final_para_plots))
    else:
    ...
    if nome_dataset_usado:
        if X_teste_df is not None and not X_teste_df.empty:
            gerar_visualizacoes_completas(
                nome_dataset_original=nome_dataset_usado,
                lista_tamanhos_explicacoes_validas=tamanhos_plot_geral,
                lista_todas_features_explicacoes_validas=features_plot,
                contagem_validacao_final=contagem_validacao_plot,
                scores_decisao_teste=scores_teste_para_plot if scores_teste_para_plot.size > 0 else None,
                t_plus=threshold_plus,
                t_minus=threshold_minus,
                features_adicionadas_f1_todas_instancias=lista_features_adicionadas_f1_plot,
                features_removidas_f2_todas_instancias=lista_features_removidas_f2_plot,
                tamanhos_explicacoes_para_correlacao=tamanhos_plot_para_correlacao,
                distancias_ao_limiar_para_correlacao=distancias_ao_limiar_validas_plot,
                total_instancias_teste=len(X_teste_df) if X_teste_df is not None else 0
            )
        else:
