...
def criar_imagem_individual(inst: dict, inst_idx: int, X_test, 
    ...
    for feat in explanation:
        ...
        if idx is not None and 0 <= idx < num_features:
            mask[idx] = x_vals[idx]  # Usar valor original do pixel
    
    mask_img = mask.reshape(img_shape)
    
    # Determinar cor do overlay baseado na categoria
    ...
    # Ordem lógica: exceção (rejeitada) primeiro, depois positiva, depois negativa
    if rejected:
        cmap = 'Purples'
        categoria = 'REJEITADA'
        cor_titulo = 'purple'
